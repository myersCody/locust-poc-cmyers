...
class QuickstartUser(HttpUser):
    ...
    def overview_page(self):
        # We should probably make a mapping or a function to build these urls dynamically
        url_options=[
            "/reports/openshift/infrastructures/aws/costs/?filter[time_scope_units]=month&filter[time_scope_value]=-2&filter[resolution]=daily&filter[service]=AmazonVPC,AmazonCloudFront,AmazonRoute53,AmazonAPIGateway",
            "/reports/aws/costs/?filter[time_scope_units]=month&filter[time_scope_value]=-2&filter[resolution]=daily&filter[service]=AmazonVPC,AmazonCloudFront,AmazonRoute53,AmazonAPIGateway",
        ]
        for url in url_options:
            self.client.get(f"{url}", name="/aws_overview")
